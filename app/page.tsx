...
export default function Home() {
  ...
  // --- 1. LÓGICA DE TEMA (SINCRONIZACIÓN) ---
  useEffect(() => {
  ...
  if (loading && !user) return <div className="h-screen flex items-center justify-center font-black bg-white dark:bg-black text-black dark:text-white text-[10px] uppercase tracking-[1em]">Cargando...</div>

  // --- 4. LOGIN / REGISTRO (CORRECCIÓN DE COLORES) ---
  if (!user) return (
        ...
        {!viewDossier ? (
          <motion.div key="inv" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center p-8 md:p-12 w-full max-w-md border-4 border-black dark:border-white shadow-[15px_15px_0px_0px_rgba(234,88,12,1)] bg-white dark:bg-black">
            ...
            <form onSubmit={handleAuth} className="space-y-4 mb-10">
              <input 
                type="email" 
                placeholder="EMAIL" 
                className="w-full p-4 border-2 border-black dark:border-white bg-transparent font-black uppercase text-xs outline-none focus:border-orange-600 text-black dark:text-white placeholder:text-black/30 dark:placeholder:text-white/30" 
                value={email} 
          ...
          <motion.div key="dos" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-white dark:bg-black">
            <div className="max-w-4xl w-full border-4 border-black dark:border-white p-8 md:p-16 shadow-[20px_20px_0px_0px_rgba(234,88,12,1)] bg-white dark:bg-black">
              ...
              <button onClick={() => setViewDossier(false)} className="px-10 py-4 bg-black text-white dark:bg-white dark:text-black font-black text-xs uppercase border-2 border-transparent hover:bg-orange-600">Cerrar</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
  ...
  // --- 5. VINCULACIÓN DE NICK (CORRECCIÓN DE COLORES) ---
  if (!profile?.minecraft_name) return (
    <div className="fixed inset-0 bg-white dark:bg-black flex items-center justify-center p-4">
      <div className="text-center space-y-10 w-full max-w-sm border-4 border-black dark:border-white p-10 shadow-[15px_15px_0px_0px_rgba(234,88,12,1)] bg-white dark:bg-black text-black dark:text-white">
        <h2 className="text-3xl font-black italic uppercase text-black dark:text-white">Identidad_Red</h2>
        <form onSubmit={async (e:any) => { 
          ...
          <input 
            name="nick" 
            required 
            placeholder="TU_NICK_MINECRAFT" 
            className="w-full bg-transparent border-2 border-black dark:border-white p-5 font-black uppercase text-center outline-none focus:border-orange-600 text-black dark:text-white placeholder:text-black/30 dark:placeholder:text-white/30" 
          />
  ...
  // --- 6. DASHBOARD (CORRECCIÓN DE TEXTOS GLOBALES) ---
  return (
    <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center p-0 md:p-10 font-sans text-black dark:text-white transition-colors duration-300">
      ...
      <header className="flex md:hidden w-full border-b-4 border-black dark:border-white p-4 items-center justify-between bg-white dark:bg-black fixed top-0 z-[60]">
        <span className="font-black italic text-xl uppercase text-black dark:text-white">Sector <span className="text-orange-600">0</span></span>
      ...
      <div className="w-full max-w-[1440px] h-full md:h-[85vh] md:border-4 md:border-black md:dark:border-white md:shadow-[25px_25px_0px_0px_rgba(234,88,12,1)] bg-white dark:bg-black flex flex-col md:flex-row overflow-hidden relative">
        ...
        {/* TOGGLE IZQ */}
        <button onClick={() => setIsNavOpen(!isNavOpen)} className="hidden md:flex absolute left-0 top-1/2 -translate-y-1/2 z-[100] bg-black text-white p-1 border-2 border-white hover:bg-orange-600 transition-all" style={{ left: isNavOpen ? '240px' : '0px' }}>{isNavOpen ? <ChevronLeft size={16}/> : <ChevronRight size={16}/>}</button>
        ...
        <main className="flex-1 flex flex-col bg-white dark:bg-black overflow-y-auto no-scrollbar relative mb-20 md:mb-0">
          <AnimatePresence mode="wait">
                ...
                <div className="flex-1 p-6 md:p-8 overflow-y-auto space-y-6 no-scrollbar">
                  {generalMessages.map((m, i) => (
                    <div key={i} className="flex flex-col gap-1">
                      ...
                      <div className="p-4 md:p-5 border-2 border-black dark:border-white text-[11px] font-bold max-w-[85%] bg-white dark:bg-black text-black dark:text-white">{m.content}</div>
                    </div>
                  ))}
                </div>
                <div className="p-4 md:p-6 border-t-4 border-black dark:border-white flex gap-3 bg-white dark:bg-black">
                  <input placeholder="TRANSMITIR..." className="flex-1 bg-transparent p-4 text-[10px] md:text-[11px] font-black outline-none border-2 border-transparent focus:border-black dark:focus:border-white uppercase text-black dark:text-white placeholder:text-black/30 dark:placeholder:text-white/30" value={messageInput} onChange={e => setMessageInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendGeneralMessage()} />
            ...
            {activeTab === 'overview' && (
              <motion.div key="ov" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col items-center justify-center p-6 text-black dark:text-white text-center">
                <p className="text-[10px] font-black opacity-30 uppercase mb-8 md:mb-12 tracking-widest text-black dark:text-white">Activos_Totales</p>
                <h2 className="text-7xl md:text-9xl font-black tracking-tighter leading-none text-black dark:text-white"><span className="text-orange-600 text-3xl md:text-5xl align-top mr-4">$</span>{profile?.balance?.toLocaleString() || 0}</h2>
              </motion.div>
            )}
            ...
            {activeTab === 'transfer' && (
              <motion.div key="tr" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col items-center justify-center p-6 text-center text-black dark:text-white">
                <form onSubmit={handleTransfer} className="w-full max-w-md space-y-8 md:space-y-10">
                  <h2 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter leading-none text-black dark:text-white">Autorizar_Bizum</h2>
                  <div className="space-y-6">
                    <input required placeholder="NICK_RECEPTOR" value={form.mcName} onChange={e => setForm({...form, mcName: e.target.value})} className="w-full bg-transparent border-2 border-black dark:border-white p-5 font-black uppercase text-center outline-none focus:border-orange-600 text-black dark:text-white placeholder:text-black/30 dark:placeholder:text-white/30" />
                    <input required type="number" placeholder="0.00" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} className="w-full bg-transparent border-2 border-black dark:border-white p-6 font-black text-6xl md:text-7xl text-center outline-none focus:border-orange-600 text-black dark:text-white placeholder:text-black/30 dark:placeholder:text-white/30" />
                  </div>
        ...
        <motion.aside 
          ...
          className="hidden md:flex border-l-4 border-black dark:border-white p-8 overflow-y-auto no-scrollbar bg-white dark:bg-black shrink-0 text-left text-black dark:text-white relative overflow-hidden"
        >
          <div className="whitespace-nowrap w-full">
            <p className="text-[10px] font-black uppercase tracking-[0.4em] mb-10 text-black/30 dark:text-white/30">Ranking_Global</p>
                ...
                <div key={u.id} className={`p-5 border-2 ${i === 0 ? 'border-orange-600 bg-orange-600/5 shadow-[4px_4px_0px_0px_rgba(234,88,12,1)]' : 'border-black/10 dark:border-white/10'} group transition-all hover:border-black dark:hover:border-white bg-white dark:bg-black`}>
                  <div className="flex justify-between items-start">
                    <div className="flex flex-col min-w-0">
                      <span className="text-[10px] font-black uppercase truncate text-black dark:text-white" style={{ color: u.name_color }}>{i + 1}. @{u.minecraft_name}</span>
        ...
        <nav className="flex md:hidden fixed bottom-0 left-0 right-0 h-20 border-t-4 border-black dark:border-white bg-white dark:bg-black z-[100] px-2 items-center justify-around">
          {[{ id: 'chat', icon: Globe }, { id: 'overview', icon: Wallet }, { id: 'transfer', icon: Send }, { id: 'ranking_mobile', icon: Trophy }].map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`p-4 transition-all ${activeTab === item.id ? 'text-orange-600 scale-125' : 'text-black/40 dark:text-white/40'}`}>
              <item.icon size={22} />
            </button>
          ))}
        </nav>
