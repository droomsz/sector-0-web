  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center p-0 md:p-10 font-sans text-black dark:text-white transition-colors duration-300">
      ...
      <header className="flex md:hidden w-full border-b-4 border-black dark:border-white p-4 items-center justify-between bg-white dark:bg-black fixed top-0 z-[60]">
        ...
        <div className="flex items-center gap-4"><button onClick={toggleTheme}>{theme === 'light' ? <Moon size={18}/> : <Sun size={18}/>}</button><button onClick={() => supabase.auth.signOut().then(() => window.location.reload())}><LogOut size={18} className="text-red-600"/></button></div>
      </header>
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'chat' && (
              <motion.div key="chat" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col h-full">
                <div className="p-4 md:p-5 border-b-4 border-black dark:border-white bg-black/5 dark:bg-white/5 font-black uppercase text-[10px] md:text-[11px] tracking-widest">Canal_Global</div>
                <div className="flex-1 p-6 md:p-8 overflow-y-auto space-y-6 no-scrollbar">
                  {generalMessages.map((m, i) => (
                    <div key={i} className="flex flex-col gap-1">
                      ...
                      <div className="p-4 border-2 border-black dark:border-white text-[11px] font-bold max-w-[85%] bg-white dark:bg-black text-black dark:text-white">{m.content}</div>
                    </div>
                  ))}
                </div>
                <div className="p-4 md:p-6 border-t-4 border-black dark:border-white flex gap-3 bg-white dark:bg-black">
                  ...
                  <button onClick={sendGeneralMessage} className="bg-black text-white dark:bg-white dark:text-black px-6 md:px-10 py-2 text-[10px] font-black uppercase hover:bg-orange-600 transition-all border-2 border-transparent">Enviar</button>
                </div>
              </motion.div>
            )}

            {activeTab === 'clans' && (
              <motion.div key="cl" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 md:p-10 space-y-12">
                {myClan ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left">
                    <div className="space-y-8">
                      <p className="text-[10px] font-black opacity-30 uppercase">Buscar_Facción</p>
                      <div className="flex border-4 border-black dark:border-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] bg-white dark:bg-black">
                        <input placeholder="NOMBRE..." className="flex-1 p-5 bg-transparent outline-none font-bold text-xs" value={clanSearchQuery} onChange={e => setClanSearchQuery(e.target.value)} />
                      ...
                      {availableClans.map(c => (
                        <div key={c.id} className="p-4 border-2 border-black dark:border-white flex justify-between items-center bg-black/5 dark:bg-white/5">
                          <span className="font-black text-[10px] uppercase">{c.name}</span>
                          <button onClick={() => joinClan(c.id)} className="text-[9px] font-black border-2 border-black dark:border-white px-3 py-1 hover:bg-orange-600 hover:text-white transition-all text-black dark:text-white">UNIRSE</button>
                        </div>
                      ))}
                    </div>
                    <div className="space-y-8">
                      <p className="text-[10px] font-black opacity-30 uppercase text-center">Fundar_Nodo</p>
                      <input placeholder="NOMBRE_NUEVO..." className="w-full p-5 border-4 border-black dark:border-white bg-transparent font-black uppercase outline-none focus:border-orange-600" value={newClanName} onChange={e => setNewClanName(e.target.value)} />
                      <button onClick={createClan} className="w-full py-6 bg-black text-white dark:bg-white dark:text-black font-black text-[10px] uppercase hover:bg-orange-600 transition-all shadow-[8px_8px_0px_0px_rgba(234,88,12,1)] border-2 border-transparent">Registrar Clan</button>
                    </div>
                  </div>
                )}
              </motion.div>
            ...
            {activeTab === 'messages' && (
              <motion.div key="ms" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col h-full bg-black/5 dark:bg-white/5">
                {!selectedUser ? (
                    ...
                    <div className="flex-1 p-6 overflow-y-auto space-y-4 no-scrollbar">
                      {privateMessages.map((m, i) => (
                        <div key={i} className={`flex flex-col ${m.sender_id === user.id ? 'items-end' : 'items-start'}`}>
                          <div className={`p-4 border-2 border-black dark:border-white text-[11px] font-bold max-w-[80%] ${m.sender_id === user.id ? 'bg-orange-600 text-white' : 'bg-white dark:bg-black text-black dark:text-white'}`}>
                            {m.content}
                          </div>
                        </div>
                      ))}
