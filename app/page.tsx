...
export default function Home() {
  ...
  const [loading, setLoading] = useState(true) // Control global de carga
  ...
  // --- 2. CARGA DE DATOS (REFORZADA PARA SESIONES PERSISTENTES) ---
      ...
      if (prof) {
        ...
        if (mem && mem.clans) {
          ...
          const { data: members } = await supabase.from('clan_members')
            ...
            .eq('clan_id', cData.id)
          if (members) setClanMembers(members.filter((m: any) => m.status === 'accepted'))
        }
      }
    }
    setLoading(false)
  ...
  const fetchOnline = useCallback(async () => {
    const { data } = await supabase.from('profiles').select('id, minecraft_name, last_seen_web, balance, name_color').not('minecraft_name', 'is', null).order('balance', { ascending: false }).limit(20)
    setOnlineUsers(data || [])
  }, [])

  const fetchGeneralChat = useCallback(async () => {
    const { data } = await supabase.from('general_messages').select('*, profiles(minecraft_name, name_color)').order('created_at', { ascending: false }).limit(50)
  ...
  useEffect(() => {
    ...
    // VIGILANTE DE AUTENTICACIÓN (CLAVE PARA VERCEL)
  ...
  // --- PANTALLA DE CARGA ---
  if (loading) return (
    <div className="h-screen flex items-center justify-center font-black bg-white dark:bg-black text-current text-[10px] uppercase tracking-[1em]">
          ...
          <motion.div key="dos" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-[999] flex items-center justify-center p-6 bg-white dark:bg-black overflow-y-auto">
            <div className="max-w-4xl w-full border-2 border-current p-8 md:p-16 shadow-[20px_20px_0px_0px_rgba(234,88,12,1)] bg-white dark:bg-black text-current text-left">
              ...
              <div className="space-y-8 font-bold text-xs uppercase border-l-4 border-orange-600 pl-8">
                <section><p className="text-orange-600">— SERVIDOR</p><p className="opacity-70">Entorno técnico de Minecraft enfocado en economía avanzada y control territorial.</p></section>
                <section><p className="text-orange-600">— PLATAFORMA</p><p className="opacity-70">Núcleo de mando sincronizado para Bizum, gestión de facciones y ranking en tiempo real.</p></section>
                <section><p className="text-orange-600">— STATUS</p><p className="opacity-70">Fase operativa activa tras finalizar los exámenes finales. Prepárate para el despliegue.</p></section>
              </div>
  ...
  return (
    <div className="central-vault bg-white dark:bg-black text-current">
      {showEditModal && (
        <div className="fixed inset-0 z-[600] bg-black/90 flex items-center justify-center p-6 text-current">
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'chat' && (
            <motion.div key="chat" className="flex-1 flex flex-col h-full border-2 border-current bg-current/[0.01]">
              <div className="p-4 border-b-2 border-current flex justify-between items-center"><span className="text-[10px] font-black uppercase">Chat_Global</span><Globe size={14} className="text-orange-600 animate-pulse" /></div>
              ...
              <div className="p-4 border-t-2 border-current flex gap-2 text-current">
              ...
              {myClan ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                  ...
                  <div className="space-y-6">
                    ...
                    <input placeholder="NOMBRE..." className="input-sharp w-full border-2 border-current bg-transparent p-4 uppercase" value={newClanName} onChange={e => setNewClanName(e.target.value)} />
          ...
          {activeTab === 'transfer' && (
            <motion.div key="tr" className="max-w-lg mx-auto py-4 w-full">
              ...
              <form onSubmit={handleTransfer} className="space-y-8 border-2 border-current p-12 bg-current/[0.02]">
                ...
                <input required type="number" placeholder="CANTIDAD" value={form.amount} onChange={e => setForm({...form, amount: e.target.value})} className="input-sharp w-full !text-6xl !font-black bg-transparent border-2 border-current p-4 text-current" />
          ...
          {activeTab === 'messages' && (
            <div className="h-full flex gap-8">
              <div className="w-64 space-y-6">
                <div className="flex border-2 border-current">
                  <input placeholder="BUSCAR..." className="flex-1 p-3 bg-transparent text-[10px] outline-none text-current" value={searchUserQuery} onChange={(e) => setSearchUserQuery(e.target.value)} />
                ...
                <div className="space-y-2">
                  {searchResults.map(p => (
                    <button key={p.id} onClick={() => openPrivateChat(p)} className={`w-full p-4 border border-current/10 text-left transition-all ${selectedUser?.id === p.id ? 'bg-orange-600 text-white' : 'hover:border-current text-current'}`}><p className="text-[10px] font-black uppercase">@{p.minecraft_name}</p></button>
                  ))}
                </div>
              </div>
              <div className="flex-1 border-2 border-current flex flex-col bg-current/[0.02]">
                {selectedUser ? (
                  <>
                    <div className="p-4 border-b-2 border-current bg-current/5 uppercase text-[10px] font-black text-current">Chat: @{selectedUser.minecraft_name}</div>
                    <div className="flex-1 p-6 overflow-y-auto space-y-4 no-scrollbar">
                      {chatMessages.map((m, i) => (
                        <div key={i} className={`flex ${m.sender_id === user?.id ? 'justify-end' : 'justify-start'}`}><div className={`p-4 text-[11px] font-bold ${m.sender_id === user?.id ? 'bg-orange-600 text-white' : 'border-2 border-current text-current'}`}>{m.content}</div></div>
                      ))}
                    </div>
                  </>
                ) : <div className="flex-1 flex items-center justify-center opacity-10 uppercase font-black text-xs text-current">Seleccionar contacto</div>}
              </div>
            </div>
          )}
        </AnimatePresence>
